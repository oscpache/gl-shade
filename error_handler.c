...
void check_and_set_Rseed(char const *argv[], int input_id)
{
    //Handling possible errors
    char *p; double conv;
    errno = 0;
    ...
    if (errno != 0 || *p != '\0' || conv < 0.0 || conv > 1.0) 
    {
        ...
        Rseed = (float)conv; //Rseed is a global variable which is declared in rand.c
}

void check_and_set_FunctionID(char const *argv[], int input_id)
{
    //Handling possible errors
    char *p; long conv;
    errno = 0;
    ...
    {
        printf("Error %d, not available objective function: input argument %d must be an integer in the interval [1,15]\n",input_id,input_id);
        exit(EXIT_FAILURE);
    } 
    else 
        ID = (int)conv; //ID (objetive function identifier) is a global variable which is declared in HeaderGPU.h
}
