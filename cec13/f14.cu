...
{ 
  ...
  while (i < popsize) 
  {
    ...
    while(t<s_size_d)
    {
      ////////////////////////Rotate vector Conflict/////////////////////////////////
      j = threadIdx.x;
      while(j<(c +s[t] - t*overlap))
      {
        if(j>=(c - t*overlap)) Z[j-(c - t*overlap)] = pop[i].x[Pvector[j]] - OvectorVec[t].col[j-(c - t*overlap)];
        j += blockDim.x;
      }
      __syncthreads();
...
double F_H(double *x)
{
  ...
  for (t=0; t<s_size; ++t)
  {
    ...
    for (j=(c - t*overlap); j<(c +s[t] - t*overlap); ++j) 
      Z[j-(c - t*overlap)] = x[Pvector[j]] - OvectorVec[t][j-(c - t*overlap)];
