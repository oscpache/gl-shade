#include <iostream>
#include <algorithm>
#include <vector>
#include <random>

/******************* Custom data types ************************/
typedef struct{
  vector<double> solution;
  double fitness;
  unsigned evals;
} LSresult;

/***************************************************************/

/*
 * Implements the MTS-LS1 indicated in MTS
 * http://sci2s.ugr.es/EAMHCO/pdfs/contributionsCEC08/tseng08mts.pdf
 * Lin-Yu Tseng; Chun Chen, "Multiple trajectory search for Large Scale Global Optimization," Evolutionary Computation, 2008. CEC 2008. (IEEE World Congress on Computational Intelligence). IEEE Congress on , vol., no., pp.3052,3059, 1-6 June 2008
 * doi: 10.1109/CEC.2008.4631210
 */
class mts_ls1
{	// Private stuff 
	int counter;
	int dim;
	unsigned maxFEs;
	unsigned maxFEs_per_round;
	float lb,ub;
	ind *child_D;

public:
	vector<double> improvement;
	vector<double> SR;
	void init(int,unsigned,unsigned,float,float);
	void search(unsigned&,double*,double&,ind&);
	LSresult mts_ls1_improve_dim(vector<double>&,double,unsigned,unsigned&,ind&);	
};


void mts_ls1::init(int dimension,unsigned maxEvals,unsigned FEs_per_application,float lowerbound,float upperbound)
{
	dim = dimension;
	maxFEs = maxEvals;
	maxFEs_per_round = FEs_per_application;
	lb = lowerbound;
	ub = upperbound;

	//cudaMalloc(&child_D,one*sizeof(ind));

	int k;
	double sr = (ub - lb) * 0.2; //Step size = 20

	// Set improvement and search range 
	for (k = 0; k < dim; ++k)
	{
		improvement.push_back(0.0);
		SR.push_back(sr);
	}

	return;
}

void mts_ls1::search(unsigned &FEs,double *best_chrom,
double &best_chrom_fx,ind &global_best)
{
	int index;
	counter = 0;
	vector<double> sol(dim);

  	//Load best chromosome
  	double best_fitness = best_chrom_fx;
  	for (index = 0; index < dim; ++index) sol[index] = best_chrom[index];

  	// Define current and result structures 
	LSresult result;
  	LSresult current_best = {sol, best_fitness, 0};

  	vector<double> dim_sorted(dim);
  	iota(dim_sorted.begin(), dim_sorted.end(), 0);

  	double improve;
  	//warm-up
  	if( counter<maxFEs_per_round && FEs<maxFEs)
  	{
	    next_permutation(dim_sorted.begin(), dim_sorted.end());
	    for( auto it = dim_sorted.begin(); it != dim_sorted.end(); it++ )
	    {
	      	result = mts_ls1_improve_dim(sol, best_fitness, *it,FEs,global_best);
	      	//counter += result.evals;
	      	improve = max(current_best.fitness - result.fitness, 0.0);
	      	improvement[*it] = improve;
	      	if( improve > 0.0 ) current_best = result;
	      	else SR[*it] /= 2.0;
		}
  	}
	iota(dim_sorted.begin(), dim_sorted.end(), 0);
	sort(dim_sorted.begin(), dim_sorted.end(), [&](unsigned i1, unsigned i2) { return improvement[i1] > improvement[i2]; });

	int i, d = 0, next_d, next_i;
	while( counter<maxFEs_per_round && FEs<maxFEs)
	{
		i = dim_sorted[d];
		result = mts_ls1_improve_dim(current_best.solution, current_best.fitness, i,FEs,global_best);
		//counter += result.evals;
		improve = max(current_best.fitness - result.fitness, 0.0);
		improvement[i] = improve;
		next_d = (d+1)%dim;
		next_i = dim_sorted[next_d];

		if( improve > 0.0 )
		{
		  current_best = result;
		  if( improvement[i] < improvement[next_i] )
		  {
		    iota(dim_sorted.begin(), dim_sorted.end(), 0);
		    sort(dim_sorted.begin(), dim_sorted.end(), [&](unsigned i1, unsigned i2) { return improvement[i1] > improvement[i2]; });
		  }
		}
		else 
		{
		  SR[i] /= 2.0;
		  d = next_d;
		  if( SR[i] < 1e-15 ) SR[i] = (ub - lb) * 0.2;
		}
	}

	sol = current_best.solution;
	// Register results
	for (index = 0; index < dim; ++index) best_chrom[index] = sol[index];
	best_chrom_fx = current_best.fitness;

	//cudaFree(child_D);

	return;
}

LSresult mts_ls1::mts_ls1_improve_dim(vector<double>& sol,
double best_fitness, unsigned i,unsigned &FEs,ind &global_best)
{
  int index;
  vector<double> newsol = sol;
  newsol[i] -= SR[i];

  //double initial_fit = best_fitness;

  // Check new solution
  if (newsol[i] > ub) newsol[i] = (ub+sol[i])/2;
  else if (newsol[i] < lb) newsol[i] = (lb+sol[i])/2;

  //f(x) on CPU with omp
  /**********************************/
  double fitness_newsol = F_H(newsol.data());//fp->compute(newsol.data()); 
  FEs += 1; counter += 1;
  if(FEs==1.2e5 || FEs==3e5 || FEs==6e5 || FEs==9e5 || FEs==1.2e6 || FEs==1.5e6
  || FEs==1.8e6 || FEs==2.1e6 || FEs==2.4e6 || FEs==2.7e6 || FEs==3e6)
  {
  	if(fitness_newsol<best_fitness)
  		fprintf(file_results,"%u,%d,%.2f,%.6e\n",FEs,ID,Rseed,fitness_newsol); 
  	else
  		fprintf(file_results,"%u,%d,%.2f,%.6e\n",FEs,ID,Rseed,best_fitness); 
  }
  /**********************************/

  unsigned evals = 1;

  if( fitness_newsol < best_fitness )
  {
    best_fitness = fitness_newsol;
    sol = newsol;

  	// Update best solution found so far
  	if (FEs<=maxFEs)
  	{
  		global_best.FEs_when_found = FEs;
  		for (index = 0; index < dim; ++index) global_best.x[index] = sol[index];
  		global_best.fx = best_fitness;
  	}
  } 
  else if( fitness_newsol > best_fitness )
  {
    newsol[i] = sol[i];
    newsol[i] += 0.5 * SR[i];

    // Check new solution
  	if (newsol[i] > ub) newsol[i] = (ub+sol[i])/2;
  	else if (newsol[i] < lb) newsol[i] = (lb+sol[i])/2;

  	//f(x) on CPU with omp
  	/**********************************/
    fitness_newsol = F_H(newsol.data());//fp->compute(newsol.data()); 
    FEs += 1; counter += 1;  
	if(FEs==1.2e5 || FEs==3e5 || FEs==6e5 || FEs==9e5 || FEs==1.2e6 || FEs==1.5e6
	|| FEs==1.8e6 || FEs==2.1e6 || FEs==2.4e6 || FEs==2.7e6 || FEs==3e6)
	{
		if(fitness_newsol<best_fitness)
		{
			fprintf(file_results,"%u,%d,%.2f,%.6e\n",FEs,ID,Rseed,fitness_newsol);  
		}
		else
		{
			fprintf(file_results,"%u,%d,%.2f,%.6e\n",FEs,ID,Rseed,best_fitness); 
		}
	}
    /**********************************/

    evals++;

    if(fitness_newsol < best_fitness )
    {
      best_fitness = fitness_newsol;
      sol = newsol;

	  // Update best solution found so far
	  if (FEs<=maxFEs)
	  {
	  	global_best.FEs_when_found = FEs;
	  	for (index = 0; index < dim; ++index) global_best.x[index] = sol[index];
	  	global_best.fx = best_fitness;
	  }
    }
  }
  
  return LSresult{sol, best_fitness, evals};
}

